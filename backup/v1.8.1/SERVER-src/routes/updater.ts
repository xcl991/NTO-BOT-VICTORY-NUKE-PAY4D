...
router.post('/download', asyncHandler(async (req, res) => {
  ...
  if (!downloadUrl || typeof downloadUrl !== 'string') {
    return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'downloadUrl is required' } });
  }
  ...
  try {
    ...
    res.status(500).json({ success: false, error: { code: 'DOWNLOAD_FAILED', message: `Download failed: ${msg}` } });
  }
}));
...
router.post('/apply', asyncHandler(async (_req, res) => {
  ...
  if (!fs.existsSync(zipPath)) {
    return res.status(400).json({ success: false, error: { code: 'NO_UPDATE', message: 'No update downloaded. Run download first.' } });
  }
...
router.post('/upload', asyncHandler(async (req, res) => {
  ...
  if (!req.files || !req.files.updateFile) {
    return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'No file uploaded. Send as updateFile.' } });
  }

  const file = req.files.updateFile as any;
  if (!file.name.endsWith('.zip')) {
    return res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Only .zip files are accepted.' } });
  }
