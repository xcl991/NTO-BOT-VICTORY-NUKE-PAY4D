...
const api = {
  ...
  bot: {
    ...
    screenshotUrl(accountId) { return API_BASE + `/bot/screenshot/${accountId}`; },
  },

  nto: {
    list(provider, limit = 50, offset = 0) { return api.get(`/nto?${provider ? `provider=${provider}&` : ''}limit=${limit}&offset=${offset}`); },
    latest(provider) { return api.get('/nto/latest' + (provider ? `?provider=${provider}` : '')); },
    stats() { return api.get('/nto/stats'); },
    check(accountId, params = {}) { return api.post('/nto/check', { accountId, ...params }); },
    exportExcel(accountId, ntoResultId) { return api.post('/nto/export', { accountId, ntoResultId }); },
    sendTelegram(accountId, ntoResultId) { return api.post('/nto/telegram', { accountId, ntoResultId }); },
    result(id) { return api.get(`/nto/result/${id}`); },
  },
