      ...
      for (const accountId of accountIds) {
        ...
        try {
          ...
          for (const uplineUsername of uplineList) {
            ...
            // Process data
            const summary = summarize(result.rows, mode, uplineUsername);
          ...
          if (companyData && lrBotToken && chatId) {
            try {
              const now = new Date();
              const companyTitle = `COMPANY REPORT — ${now.toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })} ${pad(now.getHours())}:${pad(now.getMinutes())}`;
