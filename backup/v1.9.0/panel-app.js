...
const SP_PROVIDERS = [
  { key: 'NUKE', label: 'NUKE', icon: 'fa-bolt', color: 'red', defaultUrl: 'https://cpt77.nukepanel.com', feature: 'SUMMARYPERFORMANCE' },
  { key: 'VICTORY', label: 'VICTORY', icon: 'fa-trophy', color: 'yellow', defaultUrl: '', feature: 'SUMMARYPERFORMANCE' },
  { key: 'PAY4D', label: 'PAY4D', icon: 'fa-credit-card', color: 'green', defaultUrl: '', feature: 'SUMMARYPERFORMANCE' },
];
...
window.showSection = function(sectionId, element) {
  ...
  switch (sectionId) {
    ...
    case 'sp-accounts-nuke': loadProviderAccounts('NUKE', 'SUMMARYPERFORMANCE'); break;
    case 'sp-accounts-victory': loadProviderAccounts('VICTORY', 'SUMMARYPERFORMANCE'); break;
    case 'sp-accounts-pay4d': loadProviderAccounts('PAY4D', 'SUMMARYPERFORMANCE'); break;
    case 'sp-scheduler': loadUnifiedSPScheduler(); break;
...
function renderProviderSections() {
  ...
  const allSections = [
    ...
    ...SP_PROVIDERS.map(p => ({ ...p, sectionPrefix: 'sp-accounts', idPrefix: `SP_${p.key}`, featureLabel: 'SUMMARY PERFORMANCE' })),
  ];
  ...
  for (const p of allSections) {
    const sectionId = p.sectionPrefix === 'tarikdb' ? `tarikdb-${p.key.toLowerCase()}`
      ...
      : p.sectionPrefix === 'sp-accounts' ? `sp-accounts-${p.key.toLowerCase()}`
...
async function loadDashboard() {
  try {
    ...
    // Load SUMMARYPERFORMANCE sidebar counts
    try {
      const spRes = await api.dashboard.getStats('SUMMARYPERFORMANCE');
      if (spRes.success) {
        for (const p of spRes.data.providers) {
          const el = document.getElementById(`navSp${capitalize(p.provider.toLowerCase())}Count`);
          if (el) el.textContent = String(p.total);
        }
      }
    } catch (e) { /* non-critical */ }
  } catch (e) {
...
async function loadProviderAccounts(provider, feature) {
  feature = feature || 'NTO';
  const uid = feature === 'TARIKDB' ? `TARIKDB_${provider}` : feature === 'GANTINOMOR' ? `GANTINOMOR_${provider}` : feature === 'SUMMARYPERFORMANCE' ? `SP_${provider}` : provider;
  const stateKey = feature === 'TARIKDB' ? 'tarikdbAccounts' : feature === 'GANTINOMOR' ? 'gantinomorAccounts' : feature === 'SUMMARYPERFORMANCE' ? 'spAccounts' : 'accounts';
  try {
...
function parseUid(uid) {
  ...
  if (uid.startsWith('SP_')) {
    return { provider: uid.replace('SP_', ''), feature: 'SUMMARYPERFORMANCE' };
  }
    ...
    if (settingsRes.success) {
      for (const s of settingsRes.data) {
        switch (s.key) {
          ...
          case 'notification.telegramChatIdGantiNomor': setInput('settingTelegramChatIdGantiNomor', s.value); break;
          case 'notification.telegramChatIdSummaryPerformance': setInput('settingTelegramChatIdSummaryPerformance', s.value); break;
...
// ============================================================
// Summary Performance Scheduler (Unified)
// ============================================================

async function loadUnifiedSPScheduler() {
  const section = document.getElementById('sp-scheduler');
  if (!section) return;

  const providerSections = SP_PROVIDERS.map(p => {
    const key = p.key.toLowerCase();
    return `
      <div class="bg-white rounded-xl shadow-sm p-5">
        <h3 class="font-bold text-base mb-3"><i class="fas ${p.icon} mr-2 text-${p.color}-500"></i>${p.label} Accounts</h3>
        <input type="text" placeholder="Cari account..." class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm mb-2" oninput="filterSPAccounts('${p.key}', this.value)">
        <div class="flex gap-2 mb-2">
          <button onclick="selectAllSPAccounts('${p.key}', true)" class="text-xs px-3 py-1 bg-blue-100 text-blue-700 rounded hover:bg-blue-200">Pilih Semua</button>
          <button onclick="selectAllSPAccounts('${p.key}', false)" class="text-xs px-3 py-1 bg-gray-100 text-gray-600 rounded hover:bg-gray-200">Hapus Semua</button>
          <span class="ml-auto text-xs text-gray-500" id="sp-count-${key}">0/0 dipilih</span>
        </div>
        <div id="sp-picker-${key}" class="max-h-40 overflow-y-auto border border-gray-200 rounded-lg divide-y divide-gray-100">
  ...
  section.innerHTML = `
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-3">
      <div>
        <h2 class="text-2xl font-bold"><i class="fas fa-chart-line mr-2 text-emerald-500"></i>Summary Performance</h2>
        <p class="text-sm text-gray-500 mt-1">Unified scheduler — semua provider dalam 1 report</p>
      </div>
      <div class="flex gap-2">
        <button onclick="toggleSPScheduler()" id="sp-toggle" class="px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 text-sm font-medium">
          <i class="fas fa-play mr-1"></i>Start
        </button>
        <button onclick="runSPNow()" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 mb-6">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
          <div>
            <p class="font-medium text-sm">Enable</p>
            <p class="text-xs text-gray-500">Auto-run setiap interval</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" id="sp-enabled" class="sr-only peer" onchange="saveSPUnifiedSettings()">
        ...
        <div class="p-3 bg-gray-50 rounded-lg">
          ...
          <input type="number" id="sp-interval" value="60" min="10" max="1440" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" onchange="saveSPUnifiedSettings()">
        </div>
        <div class="p-3 bg-gray-50 rounded-lg">
          <div class="flex items-center justify-between mb-1">
            <span class="text-sm font-medium">Status</span>
            <span id="sp-status" class="text-xs px-2 py-1 rounded-full bg-gray-200 text-gray-600">Stopped</span>
          </div>
          <div id="sp-lastrun" class="text-xs text-gray-500 mt-1">Belum pernah dijalankan</div>
        </div>
      </div>
      <div class="mt-3 p-3 bg-blue-50 rounded-lg text-xs text-blue-700">
        <i class="fas fa-info-circle mr-1"></i>Output: 1 combined PNG (horizontal, 1 row per account) dikirim ke Telegram SP group. NUKE &amp; VICTORY menampilkan DP Count + DP Amount + NTO/Valid Bet. PAY4D menampilkan Bet total per game category.
      </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
      ...
      <div id="sp-log" class="h-48 overflow-y-auto bg-gray-900 rounded-lg p-3 font-mono text-xs text-green-400 space-y-1">
  ...
  try {
    ...
    for (const s of settingsRes.data) {
      if (s.key === 'summaryperformance.scheduler.enabled') {
        const el = document.getElementById('sp-enabled');
        if (el) el.checked = s.value === 'true';
      }
      if (s.key === 'summaryperformance.scheduler.interval') {
        const el = document.getElementById('sp-interval');
        if (el) el.value = s.value || '60';
      }
      // gameCategories no longer needed — PAY4D now uses dedicated Performance Review page
    }

    // Load accounts for each provider
    for (const p of SP_PROVIDERS) {
      const key = p.key.toLowerCase();
      const accountsRes = await api.accounts.list(p.key, 'SUMMARYPERFORMANCE');
      const accountList = document.getElementById(`sp-picker-${key}`);
      if (!accountList) continue;

      const accounts = accountsRes.success ? accountsRes.data : [];
      if (accounts.length === 0) {
        accountList.innerHTML = `<p class="text-xs text-gray-400 text-center py-2">Belum ada akun ${p.key} SUMMARYPERFORMANCE.</p>`;
        updateSPCounter(key);
        continue;
      }

      let selectedIds = [];
      const idsSetting = settingsRes.data.find(s => s.key === `summaryperformance.${key}.scheduler.accountIds`);
      try { selectedIds = JSON.parse(idsSetting?.value || '[]'); } catch { selectedIds = []; }

      accountList.innerHTML = accounts.map(acc => {
        const checked = selectedIds.includes(acc.id) ? 'checked' : '';
        return `<div class="sp-row-${key} flex items-center gap-2 px-3 py-2 hover:bg-gray-50" data-name="${escapeHtml(acc.name.toLowerCase())}">
          <input type="checkbox" class="sp-cb-${key} h-4 w-4 rounded text-emerald-600" value="${acc.id}" ${checked} onchange="saveSPAccounts('${p.key}')">
    ...
    console.error('loadUnifiedSPScheduler error:', e);
  }
}

function updateSPCounter(key) {
  const total = document.querySelectorAll(`.sp-cb-${key}`).length;
  const checked = document.querySelectorAll(`.sp-cb-${key}:checked`).length;
  const el = document.getElementById(`sp-count-${key}`);
  if (el) el.textContent = `${checked}/${total} dipilih`;
}

window.saveSPAccounts = async function(provider) {
  const key = provider.toLowerCase();
  const ids = Array.from(document.querySelectorAll(`.sp-cb-${key}:checked`)).map(cb => Number(cb.value));
  updateSPCounter(key);
  try {
    await api.settings.update(`summaryperformance.${key}.scheduler.accountIds`, JSON.stringify(ids));
    showNotification(`${provider}: ${ids.length} akun dipilih`, 'success', 1500);
  } catch (e) { showNotification('Error: ' + e.message, 'error'); }
};

window.saveSPUnifiedSettings = async function() {
  try {
    const enabled = document.getElementById('sp-enabled')?.checked;
    const interval = document.getElementById('sp-interval')?.value || '60';
    await Promise.all([
      api.settings.update('summaryperformance.scheduler.enabled', String(!!enabled)),
      api.settings.update('summaryperformance.scheduler.interval', interval),
    ]);
    showNotification('Settings saved', 'success', 1500);
  } catch (e) { showNotification('Error: ' + e.message, 'error'); }
};

window.filterSPAccounts = function(provider, query) {
  const key = provider.toLowerCase();
  const q = query.toLowerCase();
  document.querySelectorAll(`.sp-row-${key}`).forEach(row => {
    row.style.display = !q || row.dataset.name.includes(q) ? '' : 'none';
  });
};

window.selectAllSPAccounts = function(provider, selectAll) {
  const key = provider.toLowerCase();
  document.querySelectorAll(`.sp-row-${key}`).forEach(row => {
    if (row.style.display !== 'none') {
      const cb = row.querySelector(`.sp-cb-${key}`);
      if (cb) cb.checked = selectAll;
    }
  });
  saveSPAccounts(provider);
};

async function checkSPStatus() {
  const statusEl = document.getElementById('sp-status');
  const lastRunEl = document.getElementById('sp-lastrun');
  const btn = document.getElementById('sp-toggle');
  if (!statusEl || !btn) return;

  try {
    const res = await api.get('/settings/sp-scheduler/status');
    if (res.success) {
      const d = res.data;
      if (d.running) {
        statusEl.textContent = d.executing ? 'Executing...' : 'Running';
      ...
      if (lastRunEl && d.lastRun) {
        const ts = new Date(d.lastRun.timestamp).toLocaleString('id-ID');
        lastRunEl.innerHTML = `<span class="text-emerald-600">Last run: ${ts} (${d.lastRun.resultCount || 0} accounts)</span>`;
      }
    }
  } catch (e) {
    statusEl.textContent = 'Error';
...
window.toggleSPScheduler = async function() {
  const btn = document.getElementById('sp-toggle');
  const isRunning = btn?.dataset.running === 'true';

  try {
    if (isRunning) {
      await api.post('/settings/sp-scheduler/stop');
      showNotification('Summary Performance scheduler stopped', 'warning');
    } else {
      await api.post('/settings/sp-scheduler/start');
      showNotification('Summary Performance scheduler started', 'success');
    }
    setTimeout(() => checkSPStatus(), 500);
  } catch (e) {
    showNotification('Error: ' + e.message, 'error');
  }
};

window.runSPNow = function() {
  showConfirmModal('Run Summary Performance sekarang? (Semua provider)', async () => {
    try {
      const res = await api.post('/settings/sp-scheduler/run-now');
      showNotification(res.message || 'Summary Performance triggered!', 'success');
      setTimeout(() => checkSPStatus(), 2000);
    } catch (e) {
      showNotification('Error: ' + e.message, 'error');
    }
  }, 'Run All');
...
function setupWebSocketListeners() {
  ...
  ws.on('BOT_STATUS', (data) => {
    ...
    for (const [stateKey, prefix] of [['accounts', ''], ['tarikdbAccounts', 'TARIKDB_'], ['spAccounts', 'SP_']]) {
      for (const provider of Object.keys(state[stateKey])) {
        ...
        if (acc) {
          ...
          renderAccountsTable(uid, state[stateKey][provider], prefix === 'TARIKDB_' ? 'TARIKDB' : prefix === 'SP_' ? 'SUMMARYPERFORMANCE' : prefix ? prefix.replace('_', '') : 'NTO');
  ...
  ws.on('BOT_LOG', (data) => {
    ...
    addLog(`SP_${d.provider}Log`, `[${d.accountId}] ${d.message}`, level);
    ...
    // Route Live Report logs (ALEXIS17)
    if (d.provider === 'SYSTEM' && d.message.includes('[LiveReport]')) {
      addLog('lrLogContainer', d.message, level);
    }
    // Route Summary Performance logs to unified log panel
    if (d.provider === 'SYSTEM' && d.message.includes('[SP]')) {
      addLog('sp-log', d.message, level);
    }
...
async function loadGantiNomorSettings() {
  // GANTI NOMOR now uses unified bot token — just check listener status
