...
import { nukeSummaryPerformanceFlow, type SummaryPerformanceResult } from './flows/nuke-summary-performance-flow';
import { victorySummaryPerformanceFlow, type VictorySPResult } from './flows/victory-summary-performance-flow';
import { pay4dSummaryPerformanceFlow, type Pay4dSPResult } from './flows/pay4d-summary-performance-flow';
import {
...
export class AutomationService {
  ...
  async startBot(accountId: number): Promise<void> {
    ...
    try {
      ...
      // Launch browser
      onLog(`Launching browser (headless: ${headless}${account.proxy ? ', proxy: ' + account.proxy.replace(/\/\/.*:.*@/, '//***:***@') : ''})...`);
      ...
      if (loginResult.needsOtp) {
        ...
        onLog('Waiting for OTP input from user (2 min timeout)...', 'warning');
        // Auto-timeout after 2 minutes
        setTimeout(async () => {
          try {
            const acc = await prisma.account.findUnique({ where: { id: accountId } });
            if (acc?.status === 'waiting_otp') {
              onLog('OTP timeout (2 min). Stopping bot.', 'error');
              await updateAccountStatus(accountId, 'error', { lastError: 'OTP timeout' });
  ...
  /**
   * Run TARIK DB check for an account (NUKE, VICTORY, or PAY4D).
   ...
   * PAY4D: navigates to "All User" page, scans rows for date match, clicks profile for phone.
   */
  async checkTarikDb(accountId: number, options: {
    ...
    try {
      ...
      } else if (account.provider === 'PAY4D') {
        result = await pay4dTarikDbCheckFlow(page, account.panelUrl, onLog, options);
      } else {
  ...
  /**
   * Check NUKE Summary Performance page — scrapes /report/summary-performance
   * for today and yesterday data.
   */
  async checkSummaryPerformance(accountId: number): Promise<SummaryPerformanceResult> {
    const account = await prisma.account.findUnique({ where: { id: accountId } });
    if (!account) throw new Error('Account not found');
    if (account.provider !== 'NUKE') throw new Error('Summary Performance page only available for NUKE');
    ...
    try {
      await updateAccountStatus(accountId, 'checking_nto');
      onLog('Starting Summary Performance check...');

      const result = await nukeSummaryPerformanceFlow(page, account.panelUrl, onLog, {
        twoFaSecret: account.twoFaSecret || undefined,
      });

      if (result.success) {
        await updateAccountStatus(accountId, 'running');
        onLog('Summary Performance check complete', 'success');
      } else {
        await updateAccountStatus(accountId, 'running', { lastError: result.error });
        onLog(`Summary Performance failed: ${result.error}`, 'error');
      }

      return result;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      onLog(`Summary Performance error: ${msg}`, 'error');
      await updateAccountStatus(accountId, 'running', { lastError: msg });
      return { success: false, error: msg };
    }
  }

  async checkVictorySummaryPerformance(accountId: number): Promise<VictorySPResult> {
    const account = await prisma.account.findUnique({ where: { id: accountId } });
    if (!account) throw new Error('Account not found');
    if (account.provider !== 'VICTORY') throw new Error('Victory Summary Performance only for VICTORY provider');
    ...
    try {
      await updateAccountStatus(accountId, 'checking_nto');
      onLog('Starting Victory Summary Performance check...');

      const result = await victorySummaryPerformanceFlow(page, account.panelUrl, onLog);

      if (result.success) {
        await updateAccountStatus(accountId, 'running');
        onLog('Victory Summary Performance check complete', 'success');
      } else {
        await updateAccountStatus(accountId, 'running', { lastError: result.error });
        onLog(`Victory Summary Performance failed: ${result.error}`, 'error');
      }

      return result;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      onLog(`Victory Summary Performance error: ${msg}`, 'error');
      await updateAccountStatus(accountId, 'running', { lastError: msg });
      return { success: false, error: msg };
    }
  }

  async checkPay4dSummaryPerformance(accountId: number): Promise<Pay4dSPResult> {
    const account = await prisma.account.findUnique({ where: { id: accountId } });
    if (!account) throw new Error('Account not found');
    if (account.provider !== 'PAY4D') throw new Error('PAY4D Summary Performance only for PAY4D provider');
    ...
    try {
      await updateAccountStatus(accountId, 'checking_nto');
      onLog('Starting PAY4D Summary Performance check...');

      const result = await pay4dSummaryPerformanceFlow(page, account.panelUrl, onLog);

      if (result.success) {
        await updateAccountStatus(accountId, 'running');
        onLog('PAY4D Summary Performance check complete', 'success');
      } else {
        await updateAccountStatus(accountId, 'running', { lastError: result.error });
        onLog(`PAY4D Summary Performance failed: ${result.error}`, 'error');
      }

      return result;
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      onLog(`PAY4D Summary Performance error: ${msg}`, 'error');
      await updateAccountStatus(accountId, 'running', { lastError: msg });
      return { success: false, error: msg };
    }
  }
