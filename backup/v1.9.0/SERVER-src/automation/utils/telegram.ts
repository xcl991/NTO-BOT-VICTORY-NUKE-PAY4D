...
interface TelegramConfig {
  ...
  chatIdGantiNomor?: string;   // GANTI NOMOR group chat ID
  chatIdLiveReport?: string;   // Live Report group chat ID
}

/**
 * Get Telegram config from database settings.
 * Single bot token for all features, separate chat IDs per group.
 */
export async function getTelegramConfig(): Promise<TelegramConfig | null> {
  ...
  const [chatIdTarikDbSetting, chatIdGantiNomorSetting, chatIdLiveReportSetting] = await Promise.all([
    prisma.setting.findUnique({ where: { key: 'notification.telegramChatIdTarikDb' } }),
    prisma.setting.findUnique({ where: { key: 'notification.telegramChatIdGantiNomor' } }),
    prisma.setting.findUnique({ where: { key: 'notification.telegramChatIdLiveReport' } }),
  ]);
  ...
  return {
    ...
    chatIdGantiNomor: chatIdGantiNomorSetting?.value || undefined,
    chatIdLiveReport: chatIdLiveReportSetting?.value || undefined,
  };
}
