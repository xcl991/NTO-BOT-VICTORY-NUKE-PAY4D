...
import {
  parseGantiNomorCommands,
  processGantiNomorMessage,
} from './gantinomor-listener';
import { livereportScheduler } from './livereport-scheduler';
...
class TelegramListener {
  ...
  private startPendingCleanup() {
    ...
    this.pendingCleanupTimer = setInterval(() => {
      ...
      for (const [id, req] of this.pendingRequests) {
        if (now - req.requestedAt.getTime() > TTL_MS) {
          this.pendingRequests.delete(id);
          logger.info(`[TelegramListener] Expired pending request ${id} (TTL ${TTL_MS / 1000}s)`);
        }
      }
    }, 60000); // check every 60s
  }

  private stopPendingCleanup() {
  ...
  async start(): Promise<void> {
    ...
    logger.info('Telegram bot listener started (long polling, unified NTO + TARIK DB + GANTI NOMOR)');

    const broadcast = (global as any).wsBroadcast;
    if (broadcast) {
      broadcast({
        type: 'BOT_LOG',
        data: { accountId: 0, provider: 'SYSTEM', message: 'Telegram bot listener started (NTO + TARIK DB + GANTI NOMOR)', level: 'success' },
      });
    }
   ...
   */
  private async isChatAllowed(chatId: number | string, commandType: 'NTO' | 'TARIKDB' | 'GANTINOMOR' | 'LIVEREPORT'): Promise<boolean> {
    const config = await getTelegramConfig();
    ...
    } else if (commandType === 'GANTINOMOR') {
      // GANTI NOMOR: must be from GANTI NOMOR group
      // If no GANTI NOMOR chat ID configured, not allowed
      if (!config.chatIdGantiNomor) return false;
      return chatIdStr === config.chatIdGantiNomor;
    } else if (commandType === 'LIVEREPORT') {
      // LIVE REPORT: must be from Live Report group
      // If no Live Report chat ID configured, not allowed
      if (!config.chatIdLiveReport) return false;
      return chatIdStr === config.chatIdLiveReport;
    }
    return false;
  }

  /**
   * Parse Live Report /commands from Telegram.
   * Supported: /livesend, /mkt_only, /reborn_only, /superalx
   * Returns null if not a live report command.
   */
  private parseLiveReportCommand(text: string): { command: string; label: string; uplineFilter?: string } | null {
    const t = text.trim().toLowerCase().replace(/@\S+/g, '').trim(); // strip @botname
    if (t === '/livesend') return { command: 'livesend', label: 'All Uplines (Combo)' };
    if (t === '/mkt_only' || t === '/mkt') return { command: 'mkt_only', label: 'MKT Only', uplineFilter: 'mkt' };
    if (t === '/reborn_only' || t === '/reborn') return { command: 'reborn_only', label: 'REBORN Only', uplineFilter: 'reborn' };
    if (t === '/superalx_only' || t === '/superalx') return { command: 'superalx_only', label: 'SUPERALX Only', uplineFilter: 'superalx' };
    return null;
  }
  ...
  private async handleMessage(message: any): Promise<void> {
    ...
    // Strip leading bot command prefixes (/nto or /tarik or /ganti + @botname)
    let cleanText = text;
    ...
    } else if (cleanText.match(/^\/ganti(nomor)?(@\S+)?\b/i)) {
      cleanText = cleanText.replace(/^\/ganti(nomor)?(@\S+)?\s*/i, '').trim();
    }

    const upper = cleanText.toUpperCase();

    // Route: Live Report /commands first (from Live Report group), then GANTI NOMOR, TARIK DB, NTO
    const liveReportCommand = this.parseLiveReportCommand(text);
    if (liveReportCommand) {
      if (!(await this.isChatAllowed(chatId, 'LIVEREPORT'))) {
        logger.debug(`Live Report command ignored from chat ${chatId} (wrong group)`);
        return;
      }
      logger.info(`Telegram [LIVEREPORT] command "${liveReportCommand.command}" from ${senderName}`);

      if (livereportScheduler.isExecuting()) {
        await sendTelegramMessageToChat(chatId, `⏳ <b>Live Report sedang running.</b> Tunggu selesai sebelum trigger baru.`, messageId);
        return;
      }

      await sendTelegramMessageToChat(chatId, `🚀 <b>Live Report triggered:</b> ${liveReportCommand.label}\n<i>Diminta oleh ${senderName}</i>`, messageId);

      // Run in background
      livereportScheduler.runNow(undefined, undefined, liveReportCommand.uplineFilter).catch(err => {
        logger.error(`Live Report command error: ${err}`);
        sendTelegramMessageToChat(chatId, `❌ <b>Live Report error:</b> ${err instanceof Error ? err.message : String(err)}`, messageId).catch(() => {});
      });
      return;
    }

    // Route: GANTI NOMOR first (most specific), then TARIK DB, then NTO
    if (upper.includes('GANTI NOMOR') || upper.includes('GANTINOMOR')) {
      // Check if this chat is allowed for GANTI NOMOR commands
      if (!(await this.isChatAllowed(chatId, 'GANTINOMOR'))) {
        logger.debug(`GANTI NOMOR command ignored from chat ${chatId} (wrong group)`);
        return;
      }
      logger.info(`Telegram [GANTINOMOR] command from ${senderName}: "${cleanText.substring(0, 120)}"`);
      ...
      // Queue all commands from this message together
      queueForAccount(`gantinomor_msg_${messageId}`, async () => {
        try {
          await processGantiNomorMessage(config.botToken, chatId, messageId, commands, senderName);
        } catch (err) {
          logger.error(`GANTI NOMOR message error: ${err}`);
          await sendTelegramMessageToChat(chatId, `❌ <b>Error:</b> ${err instanceof Error ? err.message : String(err)}`, messageId).catch(() => {});
        }
      });
    } else if (upper.includes('TARIKDB') || upper.includes('TARIK DB') || upper.includes('TARIK')) {
