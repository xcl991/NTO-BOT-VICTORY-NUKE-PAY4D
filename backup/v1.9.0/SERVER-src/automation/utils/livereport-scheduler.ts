...
import { victoryLiveReportFlow, victoryCompanyReportFlow, type CompanyReportRow } from '../flows/victory-livereport-flow';
import {
  summarize,
  buildTableRows,
  computeTop5,
  renderTableToPng,
  renderCombinedReportToPng,
...
class LiveReportScheduler {
  ...
  /** Manual trigger — runs immediately for all configured accounts or a single account.
   *  uplineFilter: if set, only process uplines matching this string (case-insensitive partial match).
   */
  async runNow(recapType?: 'daily' | 'weekly' | 'monthly', accountId?: number, uplineFilter?: string): Promise<void> {
    if (this._isExecuting) {
      logger.warn('[LiveReport] Already executing, skipping runNow');
      return;
    }
    if (recapType === 'daily') {
      await this.runDailyRecap(accountId, uplineFilter);
    } else if (recapType === 'weekly') {
      await this.runWeeklyRecap(accountId, uplineFilter);
    } else if (recapType === 'monthly') {
      await this.runMonthlyRecap(accountId, uplineFilter);
    } else {
      await this.runHourlyReport(accountId, uplineFilter);
    }
  }
  ...
  private async runHourlyReport(accountId?: number, uplineFilter?: string): Promise<void> {
    const now = new Date();
    const today = `${pad(now.getDate())}/${pad(now.getMonth() + 1)}/${now.getFullYear()}`;
    await this.runReport({ startDate: today, titleSuffix: '', accountId, uplineFilter });
  }

  private async runDailyRecap(accountId?: number, uplineFilter?: string): Promise<void> {
  ...
  private async runWeeklyRecap(accountId?: number, uplineFilter?: string): Promise<void> {
    const now = new Date();
  ...
  private async runMonthlyRecap(accountId?: number, uplineFilter?: string): Promise<void> {
    const now = new Date();
    // Previous month
  ...
  private async runReport(opts: {
    ...
    accountId?: number;
    uplineFilter?: string;  // if set, only process uplines matching this (case-insensitive)
  }): Promise<void> {
    ...
    try {
      // Load account IDs (single account override or all configured)
      ...
      // Get bot token (shared) + chat ID for Live Report
      ...
      if (!lrBotToken) {
        onLog('No Telegram Bot Token configured (notification.telegramBotToken)', 'warning');
        return;
      }
      if (!chatId) {
      ...
      // Load name mapping for "Nama" column
      const nameMappingSetting = await prisma.setting.findUnique({ where: { key: 'livereport.nameMapping' } });
      let nameMapping: Record<string, string> = {};
      try { nameMapping = JSON.parse(nameMappingSetting?.value || '{}'); } catch { /* ignore */ }

      // Load blacklist
      const blacklistSetting = await prisma.setting.findUnique({ where: { key: 'livereport.blacklist' } });
      let blacklist: string[] = [];
      try { blacklist = JSON.parse(blacklistSetting?.value || '[]'); } catch { /* ignore */ }
      ...
      for (const accountId of accountIds) {
        ...
        let uplineList = rawUpline.split(',').map(u => u.trim()).filter(Boolean);
        if (uplineList.length === 0) {
          onLog(`Account "${account.name}" has empty uplineUsername — skipping`, 'warning');
          continue;
        }

        // Apply upline filter if specified (e.g., from /superalx or /mkt_only command)
        if (opts.uplineFilter) {
          const filter = opts.uplineFilter.toLowerCase();
          uplineList = uplineList.filter(u => u.toLowerCase().includes(filter));
          if (uplineList.length === 0) {
            onLog(`Account "${account.name}" has no upline matching "${opts.uplineFilter}" — skipping`);
            continue;
          }
        }
        ...
        try {
          ...
          for (const uplineUsername of uplineList) {
            ...
            const blacklistSet = new Set(blacklist.map(u => u.toLowerCase()));
            const filteredRows = blacklistSet.size > 0
              ? result.rows.filter(r => !blacklistSet.has((r.cells[0] || '').trim().toLowerCase()))
              : result.rows;

            if (filteredRows.length === 0) {
              onLog(`All rows blacklisted for "${uplineUsername}"`, 'warning');
              continue;
            }

            if (filteredRows.length < result.rows.length) {
              onLog(`Blacklist: ${result.rows.length - filteredRows.length} user(s) excluded`);
            }

            // Process data
            const summary = summarize(filteredRows, mode, uplineUsername);
            ...
            // Render PNG (combined with company report if available)
            onLog('Rendering report image...');
            let pngBuffer: Buffer;
            if (companyData) {
              pngBuffer = await renderCombinedReportToPng(page, tableRows, title, top5, companyData, nameMapping);
            } else {
              pngBuffer = await renderTableToPng(page, tableRows, title, top5, nameMapping);
            }
