...
export const PAY4D_SELECTORS = {
  ...
  // ===== ALL USER PAGE (TARIK DB) =====
  allUser: {
    // "All User" navigation button
    menuButton: "button[onclick*=\"switchMenu('menuAllUser')\"]",
    menuButtonFallbacks: [
      'button.btn-info.btn-menu:has-text("All User")',
      'button:has-text("All User")',
    ],

    // Search input on All User page
    ...
    // First row date input (used to detect table loaded)
    firstRowDateInput: "table tbody tr:first-child input[type='text'][readonly]",

    // Profile button in action column (column index 8)
    profileButton: (onclick: string) => `button[onclick="${onclick}"]`,
    profileButtonGeneric: "button[onclick*='editProfilUsers']",

    // Profile detail page
    phoneInput: 'input.form-control.telpon',
    phoneInputFallbacks: [
      'input.telpon',
      "input[name='phone']",
      "input[placeholder*='telpon' i]",
      "input[placeholder*='phone' i]",
    ],

    // Back button from profile to All User list
    backButton: "button[onclick*=\"menuUsers('1')\"]",
    backButtonFallbacks: [
      'button:has-text("Back to All Users")',
      'button:has-text("Back")',
    ],

    // Pagination (numbered pages, no Next/Prev button)
    // Active page: <li class="active page-item"><a class="page-link">1</a></li>
    // Other pages: <li class="page-item" onclick="menuUsers(N)"><a class="page-link">N</a></li>
    paginationContainer: '#pagination',
    activePage: '#pagination li.active',
    pageItems: '#pagination li.page-item',
  },
  // ===== PERFORMANCE REVIEW PAGE (Summary Performance) =====
  performanceReview: {
    menuButton: "button[onclick*=\"switchMenu('menuPerformanceReview')\"]",
    menuButtonFallbacks: [
      'button.btn-info.btn-menu:has-text("Performance Review")',
      'button:has-text("Performance Review")',
    ],
    startDate: 'input.datepicker.startDate',
    startDateFallbacks: ['input.startDate', '.startDate'],
    endDate: 'input.datepicker.endDate',
    endDateFallbacks: ['input.endDate', '.endDate'],
    searchButton: '#prdateSearchButton',
    searchButtonFallbacks: [
      'input#prdateSearchButton',
      'input[value="Search"][onclick*="contentPerformanceReview"]',
    ],
    table: '.well-sm table.table-striped',
    tableBody: '.well-sm table.table-striped tbody',
    tableRows: '.well-sm table.table-striped tbody tr',
    alertHeading: '.alert-success:has-text("Performance Review")',
  },
} as const;
