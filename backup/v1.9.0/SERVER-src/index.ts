...
app.use(fileUpload({ limits: { fileSize: 50 * 1024 * 1024 }, useTempFiles: true, tempFileDir: path.join(__dirname, '../../data/updates/') }));
...
function ensureDirectories() {
  const dirs = ['data', 'data/logs', 'data/screenshots', 'data/exports', 'data/updates', 'profiles'];
...
async function start() {
  ...
  server.listen(PORT, async () => {
    ...
    console.log('  ╔═══════════════════════════════════════╗');
    console.log('  ║                                       ║');
    console.log(`  ║       BOT NTO Panel v${APP_VERSION}          ║`);
    console.log('  ║                                       ║');
    ...
    // Auto-start Summary Performance unified scheduler
    try {
      const spEnabled = await prisma.setting.findUnique({ where: { key: 'summaryperformance.scheduler.enabled' } });
      if (spEnabled?.value === 'true') {
        logger.info('Summary Performance scheduler enabled, auto-starting...');
        unifiedSPScheduler.start();
      } else {
        logger.info('Summary Performance scheduler not enabled, skipping auto-start');
      }
    } catch (e) {
      logger.warn(`Could not check Summary Performance scheduler auto-start: ${e}`);
    }
  });
}
