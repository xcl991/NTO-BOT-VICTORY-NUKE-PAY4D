...
router.post('/livereport-scheduler/run-now', asyncHandler(async (req, res) => {
  ...
  livereportScheduler.runNow(recapType).catch(e => logger.error(`[LiveReport] runNow error: ${e}`));
  res.json({ success: true, message: 'Live Report triggered, running in background' });
}));
