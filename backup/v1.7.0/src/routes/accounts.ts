...
router.get('/', asyncHandler(async (req, res) => {
  ...
  const accounts = await prisma.account.findMany({
    where,
    select: { id: true, provider: true, feature: true, name: true, panelUrl: true, username: true, pinCode: true, twoFaSecret: true, proxy: true, uplineUsername: true, status: true, lastNto: true, lastError: true, isActive: true, createdAt: true, updatedAt: true },
...
router.post('/', validate(createAccountSchema), asyncHandler(async (req, res) => {
  const { provider, feature, name, panelUrl, username, password, pinCode, twoFaSecret, proxy, uplineUsername } = req.body;
  const account = await prisma.account.create({
    data: { provider, feature: feature || 'NTO', name, panelUrl, username, password, pinCode, twoFaSecret, proxy: proxy || null, uplineUsername: uplineUsername || null },
  });
