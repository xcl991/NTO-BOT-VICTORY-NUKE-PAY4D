...
import type { LiveReportRawRow } from '../flows/victory-livereport-flow';
 ...
 */
function usernameKeyMkt(u: string): [number, string] | null {
  const clean = u.toLowerCase().replace(/_/g, '').trim();
  const m = clean.match(/teammkt(\d+)([a-z]+)/);
 ...
 */
function usernameKeyReborn(u: string): [number, string] | null {
  const clean = u.replace(/_/g, '');
  let m = clean.match(/teamreborn\s*([1-7])\s*([a-zA-Z])/i);
  if (!m) m = u.match(/teamreborn([1-7])([a-zA-Z])/i);
  if (m) return [parseInt(m[1], 10), m[2].toUpperCase()];
  return null;
}
 ...
 */
export function summarize(rows: LiveReportRawRow[], mode: LiveReportMode): TeamSummary {
  ...
  for (const row of rows) {
    ...
    const key = mode === 'MKT' ? usernameKeyMkt(user) : usernameKeyReborn(user);
    if (!key) continue;
 ...
 */
export function buildTableRows(summary: TeamSummary, mode: LiveReportMode): TableRow[] {
  const rows: TableRow[] = [];
  ...
  for (const n of sortedTeams) {
    ...
    for (const name of sortedNames) {
      const m = d[name];
      const label = mode === 'MKT' ? `TEAM ${n} - ${name}` : `${n}${name}`;
