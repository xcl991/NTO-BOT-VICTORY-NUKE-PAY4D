  ...
  async startBot(accountId: number): Promise<void> {
    ...
    try {
      ...
      switch (account.provider) {
        case 'NUKE':
          loginResult = await nukeLoginFlow(instance.page, account.panelUrl, account.username, account.password, onLog);
          break;
        case 'PAY4D':
  ...
  async checkNto(accountId: number, options: {
    ...
    try {
      ...
      if (account.provider === 'PAY4D') {
        ...
        result = await nukeNtoCheckFlow(page, account.panelUrl, onLog, options as {
          dateStart: string;
          dateEnd: string;
          usernames: string[];
