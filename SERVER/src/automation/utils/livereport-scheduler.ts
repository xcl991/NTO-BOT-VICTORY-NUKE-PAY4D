...
class LiveReportScheduler {
  ...
  async runNow(recapType?: 'daily' | 'weekly' | 'monthly', accountId?: number): Promise<void> {
    if (this._isExecuting) {
      logger.warn('[LiveReport] Already executing, skipping runNow');
      return;
    }
    if (recapType === 'daily') {
      await this.runDailyRecap(accountId);
    } else if (recapType === 'weekly') {
      await this.runWeeklyRecap(accountId);
    } else if (recapType === 'monthly') {
      await this.runMonthlyRecap(accountId);
    } else {
      await this.runHourlyReport(accountId);
    }
  }
  ...
  private async runHourlyReport(accountId?: number): Promise<void> {
    const now = new Date();
    const today = `${pad(now.getDate())}/${pad(now.getMonth() + 1)}/${now.getFullYear()}`;
    await this.runReport({ startDate: today, titleSuffix: '', accountId });
  }

  private async runDailyRecap(accountId?: number): Promise<void> {
  ...
  private async runWeeklyRecap(accountId?: number): Promise<void> {
    const now = new Date();
  ...
  private async runMonthlyRecap(accountId?: number): Promise<void> {
    const now = new Date();
    // Previous month
